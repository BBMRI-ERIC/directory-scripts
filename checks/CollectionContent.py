		...
		for collection in dir.getCollections():
			...
			if OoM and OoM > 4:
				...
				if len(subCollections) < 1:
					warnings.append(DataCheckWarning(make_check_id(self, "LargeNoSubcoll"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.INFO, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Suspicious situation: large collection (> 100,000 samples or cases) without subcollections; unless it is a really homogeneous collection, it is advisable to refine such a collection into sub-collections to give users better insight into what is stored there"))

			if OoM and OoM > 5:
				if (not 'size' in collection.keys()) or (collection['size'] == 0):
					warnings.append(DataCheckWarning(make_check_id(self, "LargeOoMOnly"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.INFO, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Suspicious situation: large collection (> 1,000,000 samples or cases) without exact size specified"))


			if any(x in types for x in ['HOSPITAL', 'DISEASE_SPECIFIC', 'RD']) and len(diags) < 1:
				warnings.append(DataCheckWarning(make_check_id(self, "DiagMissingDisease"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "No diagnoses provide for HOSPITAL or DISEASE_SPECIFIC or RD collection"))

			if len(diags) > 0 and not any(x in types for x in ['HOSPITAL', 'DISEASE_SPECIFIC', 'RD']):
				warnings.append(DataCheckWarning(make_check_id(self, "DiagTypeMismatch"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.INFO, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Diagnoses provided but none of HOSPITAL, DISEASE_SPECIFIC, RD is specified as collection type (this may be easily false positive check)"))

			if 'BIOLOGICAL_SAMPLES' in data_categories and len(materials) == 0:
				warnings.append(DataCheckWarning(make_check_id(self, "MaterialsMissing"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "No material types are provided while biological samples are collected"))

			if len(materials) > 0 and 'BIOLOGICAL_SAMPLES' not in data_categories:
				warnings.append(DataCheckWarning(make_check_id(self, "SampleCatMismatch"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Sample types advertised but BIOLOGICAL_SAMPLES missing among its data categories"))

			if 'MEDICAL_RECORDS' in data_categories and len(diags) < 1:
				warnings.append(DataCheckWarning(make_check_id(self, "MedRecDiagGap"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.WARNING, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "No diagnoses provide for a collection with MEDICAL_RECORDS among its data categories"))

			if len(diags) > 0 and 'MEDICAL_RECORDS' not in data_categories:
				warnings.append(DataCheckWarning(make_check_id(self, "DiagCatMismatch"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.WARNING, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Diagnoses provided but no MEDICAL_RECORDS among its data categories"))

			if 'RD' in types and len(diags_orpha) == 0:
				warnings.append(DataCheckWarning(make_check_id(self, "RDOrphaMissing"), "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.WARNING, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Rare disease (RD) collection without ORPHA code diagnoses"))
				if dir.issetOrphaCodesMapper():

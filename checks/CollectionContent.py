...
class CheckCollectionContents(IPlugin):
	def check(self, dir):
		...
		for collection in dir.getCollections():
			...
			if len(types) < 1:
				warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, "Collection type not provided " + collection['id'])
			...
			if OoM > 4:
				...
				if len(subCollections) < 1:
					warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.INFO, "Suspicious situation: large collection without subcollections - " + collection['id'])
					warnings.append(warning)
			...
			if((not 'data_access_fee' in collection or collection['data_access_fee'] == False) and 
				...
				warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, "No data access mode enabled and no data access policy URI provided for collection " + collection['id'])
				warnings.append(warning)
			...
			if((not 'sample_access_fee' in collection or collection['sample_access_fee'] == False) and 
				...
				warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, "No sample access mode enabled and no sample access policy URI provided for collection " + collection['id'])
				warnings.append(warning)
			...
			if 'HOSPITAL' in types or 'DISEASE_SPECIFIC' in types or 'RD' in types:
				...
				if len(diagnoses) < 1:
					warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, "No diagnoses provide for HOSPITAL or DISEASE_SPECIFIC or RD collection" + collection['id'])
					warnings.append(warning)

			if 'BIOLOGICAL_SAMPLES' in data_categories and len(materials) == 0:
				warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, "No material types are provided while biological samples are collected in " + collection['id'])
				warnings.append(warning)
			...
			if 'IMAGING_DATA' in data_categories:
				...
				if len(modalities) < 1:
					warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, "No image modalities provided for image collection " + collection['id'])
					warnings.append(warning)
				...
				if len(image_dataset_types) < 1:
					warning = DataCheckWarning("", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.WARNING, "No image dataset types provided for image collection " + collection['id'])
					warnings.append(warning)

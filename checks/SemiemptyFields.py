	...
	def check(self, dir, args):
		...
		for biobank in dir.getBiobanks():
			...
			if 'description' in biobank and descriptionTooShort(biobank['description']):
				warning = DataCheckWarning(self.__class__.__name__, "", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.ERROR, biobank['id'], "Suspiciously short description for biobank (< " + str(minDescWords) + " words)")
				warnings.append(warning)
		...
		for collection in dir.getCollections():
			if not 'description' in collection or re.search('^\s*$', collection['description']) or re.search('^\s*N/?A\s*$', collection['description']):
				warning = DataCheckWarning(self.__class__.__name__, "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, collection['id'], "Missing description for collection")
				warnings.append(warning)
			if 'description' in collection and descriptionTooShort(collection['description']):
				warning = DataCheckWarning(self.__class__.__name__, "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.ERROR, collection['id'], "Suspiciously short description for collection (< " + str(minDescWords) + " words)")
				warnings.append(warning)

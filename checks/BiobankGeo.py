				...
				if re.search (geocoords_pattern, biobank['latitude']) and re.search (geocoords_pattern, biobank['longitude']):
					if geoCodingEnabled:
						...
						try:
							...
							warnings.append(DataCheckWarning(self.__class__.__name__, "", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.WARNING, biobank['id'], DataCheckEntityType.BIOBANK, str(biobank['withdrawn']), "Reverse geocoding of the biobank  location failed (" + str(e) + ")"))
						log.info(logMessage)
			else:
				warnings.append(DataCheckWarning(self.__class__.__name__, "", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.INFO, biobank['id'], DataCheckEntityType.BIOBANK, str(biobank['withdrawn']), "Missing geographical coordinates ('latitude and/or 'longitude' attributes are empty)"))
				...
				if re.search (geocoords_pattern, collection['latitude']) and re.search (geocoords_pattern, collection['longitude']):
					if geoCodingEnabled:
						...
						try:
							...
							logMessage += " -> failed (" + str(e) + ")"
							warnings.append(DataCheckWarning(self.__class__.__name__, "", dir.getCollectionNN(collection['id']), DataCheckWarningLevel.WARNING, collection['id'], DataCheckEntityType.COLLECTION, str(collection['withdrawn']), "Reverse geocoding of the collection  location failed (" + str(e) + ")"))
						log.info(logMessage)

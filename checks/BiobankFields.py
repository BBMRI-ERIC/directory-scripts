	...
	def check(self, dir):
		...
		for biobank in dir.getBiobanks():
			if not 'juridical_person' in biobank or re.search('^\s*$', biobank['juridical_person']) or re.search('^\s*N/?A\s*$', biobank['juridical_person']):
				warning = DataCheckWarning("", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.ERROR, biobank['id'], "Missing juridical person")
			...
			if(not 'head_firstname' in biobank or re.search('^\s*$', biobank['head_firstname']) or 
				...
				warning = DataCheckWarning("", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.WARNING, biobank['id'], "Missing head person name")
				warnings.append(warning)

			if not 'head_role' in biobank or re.search('^\s*$', biobank['head_role']):
				warning = DataCheckWarning("", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.INFO, biobank['id'], "Missing head person role")
				warnings.append(warning)
			...
			if((not 'collaboration_commercial' in biobank or biobank['collaboration_commercial'] == False) and
				...
				warning = DataCheckWarning("", dir.getBiobankNN(biobank['id']), DataCheckWarningLevel.INFO, biobank['id'], "Biobank is available neither for commercial nor for non-for-profit collaboration")
				warnings.append(warning)

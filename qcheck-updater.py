    ...
    for entity_id in sorted(by_entity):
        ...
        for update in sorted(by_entity[entity_id], key=lambda item: (item.module, item.field, item.update_id)):
            print(
                ...
                f"{_render_value(update.current_value_at_export)} -> {_render_value(update.proposed_value)}"
            )
...
def run_updater(args: argparse.Namespace) -> int:
    ...
    with DirectorySession(url=args.directory_target) as session:
        ...
        if mismatch_updates:
            for update in mismatch_updates:
                logging.warning(
                    ...
                    update.entity_id,
                    update.field,
                    _render_value(update.expected_current_value),
                    _render_value(_normalize_live_row_value(live_rows[update.entity_id], update.field)),
                )
            confirm_action(
        ...
        confirm_action(
            f"Apply {len(updated_rows)} update(s) to schema {args.schema}?",
            force=args.force,
        )
        ...
        changed_rows = updated_df[updated_df["id"].astype(str).isin(entity_ids)]
        session.save_table(table="Collections", schema=args.schema, data=changed_rows)
        logging.info("Applied %d update(s) affecting %d collection(s) in schema %s.", len(updated_rows), len(entity_ids), args.schema)
    return EXIT_OK

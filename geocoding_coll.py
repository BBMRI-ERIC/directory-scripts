...
parser.add_argument('-o', '--outName', dest='outName', default='bbmri-directory-5-0-coll', help='Output file name')
...
for biobank in dir.getBiobanks():
    ...
    for coll in biobank['collections']:
        for colltypes in coll['type']:
            if colltypes['label'] == 'Population-based' or colltypes['label'] == 'Cohort':
                collTypeOK = True

    if biobank['name'] not in biobanksNameSkip and biobank['id'].split(':')[2].split('_')[0] not in biobanksCountrySkip and collTypeOK:
        biobankDict = {}
        ...
        if 'biobankSize' in biobankInputFeatures:
            try:
                ...
                for coll in biobank['collections']:
                    for colltypes in coll['type']:
                        if colltypes['label'] == 'Population-based' or colltypes['label'] == 'Cohort': 
                            collSize.append(int(coll['order_of_magnitude']['id']))
                biobankPropertiesDict['biobankSize'] = max(collSize)

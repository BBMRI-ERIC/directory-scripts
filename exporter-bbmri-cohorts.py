...
#from dataCheck import WarningsContainer
...
parser.add_argument('-a', '--aggregator', dest='aggregator', type=str, default=['Network','Entity','Country','NrCollWithSampleDonorProvided','NrCollWithFactsProvided','nrSamplesFactTables'], help='Space-separated list of the aggregators used in stdout. Accepted values: Network Entity Country')
...
df  = pd.DataFrame(columns = ['Network','Entity','Country','NrCollWithSampleDonorProvided','NrCollWithFactsProvided','nrSamplesFactTables'])
...
for biobank_cohort in bbmri_cohort_bb:
    df.loc[len(df)] = ['BBMRI_Cohort','Biobank',str(biobank_cohort['country']['id']),'NA','NA',int(0)]
...
for biobank_cohort_dna in bbmri_cohort_dna_bb:
    df.loc[len(df)] = ['BBMRI_Cohort_DNA','Biobank',str(biobank_cohort_dna['country']['id']),'NA','NA',int(0)]
...
for bbcoll_cohort in bbmri_cohort_bbcoll:
    df.loc[len(df)] = ['BBMRI_Cohort','BiobankWithCollectionInNetwork',str(bbcoll_cohort['country']['id']),'NA','NA',int(0)]
...
for bbcoll_cohort_dna in bbmri_cohort_dna_bbcoll:
    df.loc[len(df)] = ['BBMRI_Cohort_DNA','BiobankWithCollectionInNetwork',str(bbcoll_cohort_dna['country']['id']),'NA','NA',int(0)]
...
for coll_cohort in bbmri_cohort_coll:
    ...
    df.loc[len(df)] = ['BBMRI_Cohort','Collection',str(coll_cohort['country']['id']),str(nrSampDonProv),str(factsProvided),int(collsFactsSamples)]
...
for coll_cohort_dna in bbmri_cohort_dna_coll:
    ...
    df.loc[len(df)] = ['BBMRI_Cohort_DNA','Collection',str(coll_cohort_dna['country']['id']),str(nrSampDonProv),str(factsProvided),int(collsFactsSamples)]
...
columns_to_group_by = ['Network','Entity','Country','NrCollWithSampleDonorProvided','NrCollWithFactsProvided']

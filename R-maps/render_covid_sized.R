...
build_covid_sized_map <- function(points_path, iarc_path = NA_character_, output_variant = "med") {
...
save_covid_sized_formats <- function(points_path, iarc_path, output_dir, prefix) {
  cfg <- bbmri_map_config()
  bbmri_save_plot_formats_from_builder(
    build_plot = function(output_variant) {
      build_covid_sized_map(points_path, iarc_path, output_variant = output_variant)
    },
...
main <- function() {
  args <- bbmri_parse_args(list(
    ...
    output_prefix = "covid-sized"
  ))

  save_covid_sized_formats(args$input, args$iarc, args$output_dir, args$output_prefix)
}

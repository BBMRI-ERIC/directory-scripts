...
build_global_labels_map <- function(points_path, iarc_path = NA_character_, output_variant = "med") {
  ...
  bbmri_build_classic_standard_points_map(
    ...
    biobank_label_layout_variant = if (identical(output_variant, "small")) "global" else "default"
  )
}

save_global_labels_formats <- function(points_path, iarc_path, output_dir, prefix) {
  cfg <- bbmri_map_config()
  bbmri_save_plot_formats_from_builder(
    build_plot = function(output_variant) {
      build_global_labels_map(points_path, iarc_path, output_variant = output_variant)
    },
...
main <- function() {
  args <- bbmri_parse_args(list(
    ...
    output_prefix = "global-labels"
  ))

  save_global_labels_formats(args$input, args$iarc, args$output_dir, args$output_prefix)
}

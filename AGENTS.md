...
- SO2 descriptive PDF rendering requires XeLaTeX (prefer `latexmk -pdfxe`) and must fail rather than leave a partial PDF when compilation or assets are unavailable. `--output-chart-dir` is new-or-empty only and contains standalone vector chart PDFs from the same PGF/TikZ fragments; descriptive output targets must not alias inputs or one another.

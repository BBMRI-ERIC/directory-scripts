...
- SO2 descriptive schemas must validate applicability values against their referenced structured parent categories and mutually exclusive values against their declaring question. Preserve contradictory selections in counts and emit diagnostics rather than silently dropping them.
- SO2 descriptive PDF rendering invokes `xelatex` twice to resolve the table of contents and must fail rather than leave a partial PDF when compilation, writes, or assets are unavailable. Keep free-text tables reader-oriented: two-letter country code, institution, parent context only where configured for displayed rows, and no source-row column. Long bar charts must paginate with a shared zero-based count scale; pie labels must use short same-colour leaders on the nearest side. Published TeX is self-contained. Descriptive file outputs are new-file only by default; `describe --overwrite` may deliberately replace its JSON, TeX, PDF and chart directory after successful staging. `--output-chart-dir` is new-or-empty by default and contains standalone vector chart PDFs from the same PGF/TikZ source. Publish through target-directory sibling stages so cross-device targets remain atomic, and keep output targets distinct from inputs and one another.

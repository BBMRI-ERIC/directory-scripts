...
- For Node statistics, validate the whole hierarchy before pruning voting branches; test disconnected cycles and zero-count supported rows. Keep unknown source/category values distinct from zero in both stdout and XLSX, and obtain provenance from the real Directory configuration.

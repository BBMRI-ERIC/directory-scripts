...
- SO2 descriptive PDF rendering invokes `xelatex` directly and must fail rather than leave a partial PDF when compilation, writes, or assets are unavailable. Published TeX is self-contained. Descriptive file outputs are new-file only; `--output-chart-dir` is new-or-empty only and contains standalone vector chart PDFs from the same PGF/TikZ source. Publish through target-directory sibling stages so cross-device targets remain atomic, and keep output targets distinct from inputs and one another.
